...
pub fn ocr_files<'a,P>(ocr_images: &[P],tesseract_languages: &[String])  -> Result<String,OCRError> 
where P: 'a + AsRef<Path>, {
  let mut text = String::new();
  for lang in tesseract_languages {
      let parsed_lang = Lang::from_code(lang).ok_or_else(|| OCRError::LanguageError(lang.into()))?;
      info!("Trying language {}",lang);
      let mut lt = leptess::LepTess::new(None, &lang).map_err(|e| OCRError::TesseractError(format!("{}",e)))?;
      for image in ocr_images {
        let i = image.as_ref();
          lt.set_image(i.to_str().ok_or_else(|| OCRError::ImageError(format!("{:#?}",i)))?);
          let page_text = lt.get_utf8_text()?;
          text.push_str(&page_text);
      }
      if let Some(info) = detect(&text) {
          if info.lang() == parsed_lang {
              info!("Detected {} language with {} confidence",parsed_lang , info.confidence());
              return Ok(text);
          }
      };
  }
